...
namespace ImageUpload.Models
{
    ...
    public class JsonHelper
    {
        ...
        /// <param name="displayCount">output records count</param>
        /// <returns></returns>
        ...
        /// <param name="dt">DataTable</param>
        /// <param name="displayCount">output records count</param>
        /// <param name="totalcount">JSON display records count</param>
        /// <returns></returns>
        public static string CreateJsonParameters(DataTable dt, bool displayCount, int totalcount)
        {
            ...
            if (dt != null)
            {
                ...
                JsonString.Append(DataTableConvert2String(dt));
                JsonString.Append("]");
        ...
        private static StringBuilder DataTableConvert2String(DataTable dt)
        {
            StringBuilder JsonString = new StringBuilder();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    if (j < dt.Columns.Count - 1)
                    {
                        ...
                        {
                            JsonString.Append("\"JSON_" + dt.Columns[j].ColumnName.ToLower() + "\":" + dt.Rows[i][j].ToString().Trim().ToLower().Trim() + ",");
                        }
                        else if (dt.Columns[j].DataType == typeof(string))
                        {
                            JsonString.Append("\"JSON_" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" + dt.Rows[i][j].ToString().Trim().Replace("\"", "\\\"") + "\",");
                        }
                        else
                        {
                    ...
                    else if (j == dt.Columns.Count - 1)
                    {
                        ...
                        if (dt.Columns[j].DataType == typeof(bool))
                        {
                            JsonString.Append("\"JSON_" + dt.Columns[j].ColumnName.ToLower() + "\":" + dt.Rows[i][j].ToString().Trim().ToLower());
                        }
                        else if (dt.Columns[j].DataType == typeof(string))
                        {
                            JsonString.Append("\"JSON_" + dt.Columns[j].ColumnName.ToLower() + "\":" + "\"" + dt.Rows[i][j].ToString().Trim().Replace("\"", "\\\"") + "\"");
        ...
        /// <param name="dt">DataTable</param>
        /// <returns></returns>
        public static string CreateJsonComboBoxParameters(DataTable dt)
        {
            StringBuilder JsonString = new StringBuilder();
            //Exception Handling        
